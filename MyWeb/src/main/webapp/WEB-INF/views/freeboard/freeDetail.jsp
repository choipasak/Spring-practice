            ...
            <div class="col-xs-12 col-md-9 write-wrap">
                ...
                <form action="${pageContext.request.contextPath}/freeboard/modPage" method="post">
                    ...
                    <div class="form-group">
                        <label>번호</label> <input class="form-control" name='bno' readonly value="${gocontent.bno}">
                    </div>
                    <div class="form-group">
                        <label>작성자</label> <input class="form-control" name='writer' readonly
                            value="${gocontent.writer}">
                    </div>
                    <div class="form-group">
                        <label>제목</label> <input class="form-control" name='title' readonly value="${gocontent.title}">
                    </div>

                    <div class="form-group">
                        <label>내용</label>
                        <textarea class="form-control" rows="10" name='content' readonly>${gocontent.content}</textarea>
                    </div>

                    <button type="submit" class="btn btn-primary">변경</button>
                    <button type="button" class="btn btn-dark"
                        onclick="location.href='${pageContext.request.contextPath}/freeboard/freeList?pageNo=${p.pageNo}&amount=${p.amount}&keyword=${p.keyword}&condition=${p.condition}'">목록</button>
                    ...
                    <div class="reply-content">
                        ...
                        <div class="reply-group">
                            <div class="reply-input">
                                <input id="replyId" type="text" class="form-control" placeholder="이름"> <input
                                    id="replyPw" type="password" class="form-control" placeholder="비밀번호">
                            </div>
...
<script>
    window.onload = function () {
        ...
        document.getElementById('replyRegist').onclick = function () {
            ...
            const bno = '${gocontent.bno}'; //그냥 input태그의 value 값 자체를 가져옴(글 번호)
            const reply = document.getElementById('reply').value; // textarea값
            const replyId = document.getElementById('replyId').value; // 댓글 등록자 이름
            const replyPw = document.getElementById('replyPw').value; // 댓글 등록자 비번
            ...
            const reqObj = {
                ...
                body: JSON.stringify({ //JSON.stringify()안에 객체를 작성해 줘야 JSON의 형태로 변환된다. {}안의 내용: JS문법
                    'bno': bno,
                    'replyText': reply,
                    'replyId': replyId,
                    'replyPw': replyPw
            ...
            fetch('${pageContext.request.contextPath}/reply', reqObj)
                .then(res => res.text()) //컨트롤러에서 return한 값 = "regSuccess"
                .then(data => {
                    console.log('통신 성공! -> ' + data);
                    //등록 성공 했다면, 다음 댓글 등록을 위해 댓글 입력창을 비워주자!
        ...
        document.getElementById('moreList').onclick = () => {
            getList(++page, false);
            //++page: 페이지 먼저 올리고 보내라! -> 다음 5개 댓글 페이지 달라는 요청.
            //왜 false를 주냐 : 더보기여서 -> 누적해서 보여줘야 하니까 -> 초기화 하면 안됨!
        }

        let page = 1; //전역의 의미로 사용할 페이지 번호.
        let strAdd = ''; //화면에 그려 넣을 태그를 문자열의 형태로 추가 할 변수.
        ...
        //요청된 페이지 번호와, 화면을 리셋할 것인지의 여부를 boolean 타입의 reset으로 받겠습니다.
        //비동기 방식이기 때문에 페이지가 계속 머물면서 댓글이 밑에 쌓입니다.
        //때에 따라서는 댓글을 계속 누적 시키는 것이 아니라, 화면을 초기화 하고 새롭게 보여줘야 할 때가 있다.
        //reset -> true: 페이지를 리셋해서 새롭게 그려내기, reset -> false: 누적해서 쌓기.
        //reset = true -> 원래 보여주던 댓글 없애고 그 다음 댓글 5개를 보여줌
        //reset = false -> 기존의 댓글 살려놓고 누적해서 댓글 5개 더 보여줌
        //그래서 이제부터 getList()는 매개 변수를 2개 받는ㄷㅏ!
        //그럼 reset을 getList() 안에서 true냐 false냐 를 판단 해 줘야 한다. -> reset은 설정하는 사람의 마음대로 정하는 것. -> 상황: 누적 할지 말지
        function getList(pageNum, reset) { //pageNum: getList()를 부르면 게시글의 페이지 번호를 달라!
            console.log('getList() 호출됨!');

            const bno = '${gocontent.bno}'; //현재 게시글 번호
            console.log('bno: ' + bno);


            //요청 -> GET : 댓글 목록 요청! (비동기)
            fetch('${pageContext.request.contextPath}/reply/list/' + bno + '/' + pageNum) // /reply/list/bno값
                .then(res => res.json())
                .then(data => {
                    console.log('서버가 전달한 map: ', data);
                    strAdd = ''; // 새로운 내용을 작성하기 위해서 변수의 값을 비우기!
                    ...
                    for (let i = 0; i < replyList.length; i++) { //댓글 5개씩 반복해서 꺼내서 데이터를 보내 줄 반복문
                        //약간 노가다, 백틱을 열어서 안에 내용을 넣어 줄 것임.
                        ...
                        //위처럼 작성하기 싫어서 리액트(라이브러리)를 사용함! 
                    }

                    //id가 replyList라는 div 영역에 문자열 형식으로 모든 댓글을 추가.
                    //document.getElementById('replyList').innerHTML = strAdd; //innerHTML은 사용이 권장되지 않음

                    //댓글을 추가할 때 확인해야함 -> reset이라는 변수의 값이 false or true를
                    if (!reset) {
                        //false라면
                        //.insertAdjacentHTML() : 매개 값을 2개 받음, (삽입할 위치, 삽입할 값)
                        //insertAdjacentHTML(): 인접한 곳에 HTML을 삽입하는 함수.
                        //(position(문자열), 문자열 형태의 HTML) -> 캡처 사진
        ...
        */
        ...
        function parseTime(regDate) {
            //원하는 날짜로 객체를 생성.
            const now = new Date(); //date에는 지금. 날짜와 지금. 시간이 들어감! => 현재 날짜!
            const regTime = new Date(regDate); // 현재 시간!

            //console.log('now: ', now);
            //console.log('regTime: ', regTime);

            //.getTime() : 1970년 1월 1일 자정을 기준으로 현재까지의 시간을 밀리초로 리턴. (= 자바의 currentTimeRealease)
            const gap = now.getTime() - regTime.getTime();

            let time;
            if (gap < 60 * 60 * 24 * 1000) { //밀리초 -> 초 단위 -> 분 -> 시 -> 하루 : 차이가 하루보다 작냐?
                if (gap < 60 * 60 * 1000) { // gap이 1시간보다 작니? 
                    time = '방금 전';
                } else {
                    time = parseInt(gap / (1000 * 60 * 60)) + '시간 전';
                }
            } else if (gap < 60 * 60 * 24 * 30 * 1000) { //gap이 한달 지났니?
